...
void visualiser::broadPhase(size_t * _index, ngl::Vec3 _min, ngl::Vec3 _max, const std::vector<sphere *> &_nodes, unsigned short _lvl)
{
	...
	if(count <= 4 or _lvl > 5)
	{
		...
		if(*_index >= m_partitions.size())
			std::cout << "About to modify at " << *_index << " of " << m_partitions.size() << '\n';
		std::lock_guard<std::mutex> block_threads_until_finish_this_job(barrier);
		m_partitions[*_index] = outNodes;
		//std::lock_guard<std::mutex> block_threads_until_finish_this_job(barrier);
		*_index += 1;
	}
	else
		...
		for(int i = 0; i < 2; ++i)
			for(int j = 0; j < 2; ++j)
				for(int k = 0; k < 2; ++k)
				{
					...
					{
						threads[i * 4 + j * 2 + k] = std::thread( &visualiser::broadPhase, this, _index, newMin, newMin + dim, outNodes, _lvl + 1 );
						m_curThreads++;
					}
					else
						broadPhase(
...
void visualiser::update(const float _dt)
{
	...
	m_partitions.assign( m_nodes.size() * 8 , {} );

	sim_time dt (0.0f);
	dt.setStart();
	size_t startIndex = 0;
	broadPhase( &startIndex, initBox.first, initBox.second, initNodes, 0 );
	dt.setCur();
