	...
	m_clip(0.01f, 3500.0f)
{
	...
	if(gint("win_width") == -1 and gint("win_height") == -1)
	{
	...
	createShaderProgram( "blinn", "MVPUVNVertDistort", "blinnFrag" );
...
void visualiser::drawSpheres()
{
	...
	slib->use( "blinn" );
	slib->setRegisteredUniform1f("iGlobalTime", m_timer.getTime());
	for(auto &i : m_nodes.m_objects)
	{
...
void visualiser::finalise()
{
	...
	slib->setRegisteredUniform( "cutout", m_cutout );
...
void visualiser::update(const float _dt)
{
	...
	m_cam.setNear( m_clip.first );
	m_cam.setFar( m_clip.second );
...
void visualiser::togglePause()
{
	m_paused = !m_paused;
	if(m_paused)
		Mix_Pause( m_sdlchannel );
	else
		Mix_Resume( m_sdlchannel );
