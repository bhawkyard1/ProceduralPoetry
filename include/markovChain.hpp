...
void markovChain<T>::constructVisualisation()
{
    ...
    std::vector<            std::vector<                std::vector<note> > > connections;
    ...
    for(auto &state : m_states)
    {
        ...
        std::vector< std::vector<note> > name = state.first;
    ...
    for(size_t i = 0; i < connections.size(); ++i)
    {
        ...
        {
            std::vector< std::vector<note> > vnodeName = (*vnodes)[i].getName();
...
void markovChain<T>::loadSource(const std::string _path)
{
    ...
    while(!done)
    {
        ...
        //smpl.denoise( &data, g_PARAM_DENOISE_MUL );
